    ...
    <div class="container-custom max-w-3xl">
      <div class="card card-body">
        ...
        <div class="mb-8">
          ...
          <p class="text-gray-600">Update your story</p>
        </div>
        ...
        <form v-else @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="flex gap-4 pt-4">
            ...
            >
              <span v-if="!newsStore.loading">Update News</span>
...
import type { PutNewsCommand, TagDto } from '../../types/news.types';
...
const form = reactive<PutNewsCommand>({
  title: '',
  description: '',
  tags: [],
  image: '',
...
const errors = ref<Partial<Record<keyof PutNewsCommand, string>>>({});
const tagInput = ref('');
...
const loadNews = async () => {
  try {
    loading.value = true;
    // Находим новость в списке или загружаем
    const news = newsStore.newsList.find(n => n.id === newsId);
    ...
    if (news) {
      ...
      if (news.image) {
        imagePreview.value = fileApi.getFileUrl(news.image);
      }
    } else {
...
const handleSubmit = async () => {
  ...
  try {
    await newsStore.updateNews(newsId, form);
    router.push('/news');
  } catch (error) {
